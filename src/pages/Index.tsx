...
const PRODUCT_SIZES: ProductSize[] = [
  { title: "Single Bed", size: "single", price: 1900, dimensions: ["72x36"] },
  { title: "Double Bed", size: "double", price: 2200, dimensions: ["72x48"] },
  { title: "Queen Size", size: "queen", price: 3400, dimensions: ["60x72", "60x75", "60x78"] },
  { title: "King Size", size: "king", price: 3600, dimensions: ["72x72", "72x75", "72x78"] },
];
...
const Index = () => {
  ...
  const Cover = () => (
    <div className="w-full flex justify-center items-center mb-0 mt-8 animate-fade-in">
      <img
        ...
        style={{ background: "linear-gradient(135deg,#fff,#eee 55%,#e2d1c3 100%)" }}
      />
    </div>
  );
  ...
  return (
    <div className="min-h-screen w-full overflow-x-hidden">
      <Header />
      {/* Hero Section */}
      <section className="relative min-h-[60vh] hero-gradient px-2 py-3 md:py-9 pt-14 md:pt-20" style={{paddingBottom: 0}}>
        <motion.div
        ...
        >
          ...
          <h1 className="font-bhues text-[2.6rem] sm:text-5xl font-bold italic mb-2">
            Design Your Bed Premiumly
          </h1>
          ...
          <div className="flex justify-center items-center gap-2 mb-0 mt-3">
            {[
              ...
              >
                <div className={
                  "relative flex items-center justify-center w-9 h-9 rounded-full border-2 " +
                ...
                {i !== 2 && (
                  <div className="absolute left-full mx-1 w-6 h-0.5 bg-primary/20" />
                )}
                <span
              ...
              <>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-1">
                  {PRODUCT_SIZES.map((sizeObj) => (
                    <motion.div
                      key={sizeObj.size}
                      whileHover={{ scale: 1.065, boxShadow: "0 6px 32px 0 rgba(180,150,125,0.15)" }}
                      whileTap={{ scale: 0.97 }}
                      className={`glass p-4 md:p-5 rounded-xl card-animate cursor-pointer flex flex-col items-center justify-center hover:shadow-accent transition-all duration-200 ${
                        selectedSize === sizeObj.size ? "ring-2 ring-primary border-primary scale-105 shadow-xl" : ""
                      }`}
                      onClick={() => {
              ...
              <>
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-3 mb-1">
                  {(getProductData(selectedSize)?.dimensions ?? []).map((dimension) => (
                    <motion.div
                      key={dimension}
                      whileHover={{ scale: 1.07, boxShadow: "0 8px 28px 0 rgba(180,150,125,0.10)" }}
                      whileTap={{ scale: 0.92 }}
                      className={`glass p-4 md:p-4 rounded-xl card-animate cursor-pointer flex flex-col items-center justify-center transition-all duration-200 ${
                        selectedDimension === dimension ? "ring-2 ring-primary border-primary scale-105 shadow-xl" : ""
                      }`}
              ...
              <>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mb-1">
                  {[
                    ...
                    <motion.div
                      key={treatment.id}
                      whileHover={{ scale: 1.07, boxShadow: "0 8px 28px 0 rgba(180,150,125,0.08)" }}
                      whileTap={{ scale: 0.95 }}
                      className={`glass p-5 rounded-xl card-animate cursor-pointer transition-all duration-200 flex flex-col items-center ${
                        selectedTreatment === treatment.id ? "ring-2 ring-primary border-primary scale-105 shadow-xl" : ""
                      }`}
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            {[
              ...
              <motion.div
                ...
                transition={{ delay: index * 0.20, duration: 0.6, type: "spring" }}
                whileHover={{ scale: 1.065, rotate: 1.5 }}
